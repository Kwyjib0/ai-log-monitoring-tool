...
from io import StringIO, BytesIO  # Import StringIO/BytesIO for in-memory buffer operations
...
# if upload option is selected
    ...
    if uploaded_file is not None:  # check if file was uploaded
        # Use file name + byte-length as a stable key across reruns instead of object id
        try:
        ...
        current_file_key = (getattr(uploaded_file, 'name', None), len(uploaded_bytes))

        if st.session_state.uploaded_file_id != current_file_key:  # check if this is a new file upload
            # Parse CSV from bytes (prefer BytesIO for pandas)
            try:
                st.session_state.df = pd.read_csv(BytesIO(uploaded_bytes))
            except Exception:
                # Fallback: decode to text and use StringIO
                try:
                    st.session_state.df = pd.read_csv(StringIO(uploaded_bytes.decode('utf-8', errors='replace')))
                except Exception:
                    st.session_state.df = pd.DataFrame()

            st.session_state.uploaded_file_id = current_file_key  # store key to track this upload
            st.session_state.result_df = None  # clear previous anomaly detection results
            st.session_state.anomalies = None  # clear previous anomalies
            st.success("File uploaded successfully!")  # display success message

        # Safely display counts/head even if df could not be parsed
        if st.session_state.df is not None and len(st.session_state.df) > 0:
            ...
            st.write("Uploaded file parsed to empty DataFrame.")
...
if st.session_state.result_df is not None:  # check if results exist
    # Load result and anomalies from session state with defensive fallbacks
    result_df = st.session_state.result_df
    # Ensure anomalies is always a DataFrame (avoid None causing len(None) errors on rerun)
    if isinstance(st.session_state.anomalies, pd.DataFrame):
        anomalies = st.session_state.anomalies
    else:
        try:
            anomalies = result_df[result_df['anomaly'] == 'anomaly'] if result_df is not None else pd.DataFrame()
        except Exception:
            anomalies = pd.DataFrame()

    total = len(result_df) if result_df is not None else 0  # calculate total safely
    num_anomalies = len(anomalies) if isinstance(anomalies, (pd.DataFrame, list, tuple)) else 0  # safe count
    anomaly_rate = (num_anomalies / total) * 100 if total > 0 else 0.0  # avoid division-by-zero
    ...
    # Only show status summary if column exists and result_df is non-empty
    if result_df is not None and 'status_code' in result_df.columns and len(result_df) > 0:
        ...
        st.write("No `status_code` column available or no data to summarize.")
    ...
    # Safely write CSV even if result_df is empty or None
    try:
        if result_df is not None:
            result_df.to_csv(csv_buffer, index=False)  # write result dataframe to CSV buffer
        else:
            pd.DataFrame().to_csv(csv_buffer, index=False)
    except Exception:
        # fallback to an empty CSV
        pd.DataFrame().to_csv(csv_buffer, index=False)
